...
The exit trace added here is diagnostics-only. It records why the existing
policy evaluated a position the way it did without changing any threshold.
"""
...
class ExitPolicyPositionRiskManager(PositionRiskManager):
    ...
    def _record_exit_trace(self, position: Position, decision: PositionExitDecision) -> None:
        ...
        This does not alter the exit decision or any risk threshold. The trace
        lives with the position so it survives repository persistence and can
        later be joined to position_id and closed-position history.
        """
        now = time.time()
        ...
        trace = {
            ...
            "decision": "SELL" if decision.should_exit else (
                "REVIEW_REQUIRED" if decision.review_required else "HOLD" if decision.hold_reason else "NONE"
            ),
