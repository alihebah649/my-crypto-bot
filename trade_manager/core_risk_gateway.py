...
class CoreRiskGateway:
    ...
    def approve(self, request: RiskSizingRequest) -> RiskSizingApproval:
        try:
            ...
            sizing = self.position_sizer.calculate(account_equity=request.account_equity, entry_price=request.entry_price,
                                                   stop_loss=request.stop_loss, leverage=1.0)
            ...
            if prospective_exposure > max_exposure:
                return self._reject(RiskRejectReason.MAX_PORTFOLIO_EXPOSURE.name, metadata={
                    "current_exposure": current_exposure, "position_value": position_value,
                    "prospective_exposure": prospective_exposure, "max_exposure": max_exposure})
            ...
            if total_required > request.free_balance:
                return self._reject("INSUFFICIENT_BALANCE", metadata={"capital_required": capital_required,
                    "estimated_fee": max(request.estimated_fee, 0.0), "maintenance_margin": max(request.maintenance_margin, 0.0),
                    "free_balance": request.free_balance})
            return RiskSizingApproval(approved=True, reason="APPROVED", quantity=quantity, position_value=position_value,
                capital_required=capital_required, risk_amount=sizing.risk_amount, stop_distance=sizing.stop_distance, leverage=1.0,
                metadata={"risk_percent": config.risk_per_trade_percent, "target_position_value": config.target_position_value,
                          "source": "TradeManager.Part6", "estimated_fee": max(request.estimated_fee, 0.0)})
