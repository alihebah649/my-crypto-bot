...
class PositionManagementFacade:
    ...
    def _approve_entry(self, *, symbol: str, quantity: float, entry_price: float,
        ...
        try:
            approved = bool(self.risk_approval(symbol=symbol, quantity=quantity, entry_price=entry_price,
    ...
    def open_position(self, symbol: str, quantity: float, entry_price: float, stop_loss: float,
        ...
        self.last_entry_diagnostic = {
            "symbol": symbol, "validation": "PASS", "risk_gateway": "NOT_RUN",
            "execution_gateway": "NOT_RUN", "result": "UNKNOWN",
        }
        if self.execution_gateway is None:
            self.last_entry_diagnostic.update({"validation": "REJECT", "result": "EXECUTION_GATEWAY_NOT_WIRED"})
            return None
        ...
        if allowed_quantity <= 0:
            self.last_entry_diagnostic.update({"risk_reason": "ZERO_ALLOWED_QUANTITY",
                                                "result": "REJECTED_ZERO_QUANTITY"})
            return None
        ...
        self.last_entry_diagnostic["execution_outcome"] = {
            "success": outcome.success, "outcome": outcome.outcome.value,
        ...
        if not outcome.success or outcome.executed_quantity <= 0 or outcome.average_price <= 0:
            self.last_entry_diagnostic.update({"execution_reason": outcome.message or outcome.outcome.value,
                                                "result": "REJECTED_OR_FAILED_AT_EXECUTION"})
            return None
    ...
    def close_position(self, position_id: str, exit_price: float,
        ...
        decision = PositionExitDecision(True, PositionExitReason.REVIEW_REQUIRED, exit_price,
