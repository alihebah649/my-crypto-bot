...
def test_daily_loss_limit_locks_only_at_configured_threshold():
    ...
    below_limit = controller.evaluate(
        account=_account(), symbol="BTCUSDT", signal="SCALP", market=_market()
    )
...
def test_daily_lock_can_be_explicitly_reset_for_new_period():
    ...
    # The runtime is responsible for period-boundary reset; clearing the
    # persisted daily loss alone must not silently unlock the same day.
    tracker.update(daily_pnl=0.0, weekly_pnl=0.0, monthly_pnl=0.0)
    still_locked = controller.evaluate(
        account=_account(), symbol="BTCUSDT", signal="SCALP", market=_market()
    )
