...
class LegacyStub:
    def __init__(self):
        ...
        def score_symbol(symbol, ticker, candles_15m, candles_5m):
            return {"symbol": symbol, "score": 72, "scalp_score": 72, "scalp_signal": "BUY"}
...
def _candle(open_ms, close_ms):
    return {"open_time": open_ms, "close_time": close_ms, "open": 100.0, "high": 101.0, "low": 99.0, "close": 100.5, "volume": 10.0}


def test_score_result_contains_exact_5m_decision_candle_freshness():
    now = time.time()
    candles = [_candle(int((now - 700) * 1000), int((now - 400) * 1000)), _candle(int((now - 399) * 1000), int((now - 80) * 1000)), _candle(int((now - 79) * 1000), int((now + 221) * 1000))]
    cache = {("BTCUSDT", "5m", 60): (now - 20.0, candles)}
    legacy = LegacyStub()
    install(legacy=legacy, kline_cache=cache, kline_cache_lock=threading.RLock(), kline_cache_ttl={"5m": 310.0})
...
def test_score_result_marks_old_snapshot_stale_without_changing_signal():
    now = time.time()
    candles = [_candle(int((now - 1600) * 1000), int((now - 1300) * 1000)), _candle(int((now - 1299) * 1000), int((now - 999) * 1000)), _candle(int((now - 998) * 1000), int((now - 698) * 1000))]
...
def test_trace_prefers_persistent_market_data_manager_cache():
    now = time.time()
    candles = [_candle(int((now - 500) * 1000), int((now - 350) * 1000)), _candle(int((now - 349) * 1000), int((now - 80) * 1000)), _candle(int((now - 79) * 1000), int((now + 221) * 1000))]
    legacy = LegacyStub()
    ...
    legacy_kline_cache = {("BTCUSDT", "5m", 60): (now - 700.0, candles)}
    install(legacy=legacy, kline_cache=legacy_kline_cache, kline_cache_lock=threading.RLock(), kline_cache_ttl={"5m": 310.0})
    result = legacy.score_symbol("BTCUSDT", {"lastPrice": 100.5}, candles, candles)
    audit = result["entry_freshness_5m"]
