...
class _Risk:
    def evaluate(self, position):
        return PositionExitDecision(True, PositionExitReason.TAKE_PROFIT, 101.0, "test exit")


class _Repository:
    def __init__(self): self.positions = {}
    def add(self, position): self.positions[position.position_id] = position
    def update(self, position): self.positions[position.position_id] = position
    def get(self, position_id): return self.positions.get(position_id)
    def get_all(self): return list(self.positions.values())
    def get_open_positions(self): return [p for p in self.positions.values() if p.status in {PositionStatus.OPEN, PositionStatus.HOLD}]
    def get_by_symbol(self, symbol): return [p for p in self.positions.values() if p.symbol == symbol]


class _Calculator:
    def break_even_price(self, position): return position.entry_price
...
def test_closed_position_is_archived_with_entry_and_exit_metadata(tmp_path):
    repo = _Repository()
    history = PositionHistoryService(repository=PositionHistoryRepository(str(tmp_path / "position_history.json")))
    controller = PositionController(_Risk(), repo, _Gateway(), history_service=history)
    position = Position(position_id="pos-1", symbol="BTCUSDT", side=PositionSide.LONG,
    ...
    repo.add(position)
    controller.execute_exit_decision("pos-1", PositionExitDecision(True, PositionExitReason.TAKE_PROFIT, 101.0, "test exit"), _Calculator())
...
def test_failed_exit_does_not_archive_position(tmp_path):
    ...
    position = Position(position_id="pos-2", symbol="BTCUSDT", side=PositionSide.LONG,
                        status=PositionStatus.OPEN, quantity=1.0, entry_price=100.0,
                        current_price=100.0, stop_loss=98.0, take_profit=101.0)
    repo.add(position)
    controller.execute_exit_decision("pos-2", PositionExitDecision(True, PositionExitReason.TAKE_PROFIT, 101.0, "test exit"), _Calculator())
