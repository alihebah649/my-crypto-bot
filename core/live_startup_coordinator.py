...
class LiveStartupCoordinator:
    ...
    def _exact_protection_ok(adapter, positions: tuple[LocalPositionView, ...]) -> bool:
        for position in positions:
            ...
            matched = False
            for order in orders:
                if str(order.get("side", "")).upper() != "SELL": continue
                if str(order.get("status", "")).upper() not in {"NEW", "PARTIALLY_FILLED", "PENDING_NEW"}: continue
                if str(order.get("type", "")).upper() not in {"STOP_LOSS_LIMIT", "STOP_LOSS"}: continue
                if abs(float(order.get("origQty", 0.0) or 0.0) - float(position.quantity)) > 1e-8: continue
                if position.stop_price is not None:
                    stop = float(order.get("stopPrice", 0.0) or 0.0)
                    expected = float(position.stop_price)
                    if stop <= 0 or abs(stop - expected) > max(abs(expected) * 1e-8, 1e-12): continue
                matched = True
                break
    ...
    def start(self, local_positions: Iterable[LocalPositionView]) -> LiveStartupResult:
        ...
        if snapshot.result.safe_to_resume and not self._exact_protection_ok(self._adapter, local):
