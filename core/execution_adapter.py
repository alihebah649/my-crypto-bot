...
class ExecutionAdapter(ABC):
    """
    الواجهة المجردة لجميع الوسطاء.

    أي وسيط (Binance - Bybit - Paper ...)
    يجب أن يرث منها.
    """
...
class BinanceExecutionAdapter(ExecutionAdapter):
    ...
    def _require_client(self) -> Client:
        ...
        if self._client is None:
            ...
            raise ExchangeConnectionError(
                "Binance client is not connected."
            )
    ...
    def execute(
        ...
        params: dict[str, Any] = {
            ...
            "side": self._convert_side(request.side),
            "type": self._convert_order_type(
                request.order_type
            ),
        }
        ...
        if avg_price == 0.0:
            ...
            cumulative = self._safe_float(
                raw.get("cummulativeQuoteQty")
            )

            if (
                cumulative > 0.0
                and executed_qty > 0.0
            ):
