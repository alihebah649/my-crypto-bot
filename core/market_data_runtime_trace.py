...
def install(*, legacy: Any, kline_cache: dict, kline_cache_lock: threading.RLock, kline_cache_ttl: dict[str, float]) -> Callable[[], dict]:
    """Install diagnostic tracing on the active market-data path only."""
    ...
    def _cache_observation(symbol: str, interval: str, limit: int, now: float) -> dict[str, Any]:
        ...
        if manager is not None:
            try:
                cached = manager.cache.get(key); policy = manager.policies.get(interval_key); ttl = None if policy is None else float(policy.fresh_ttl_seconds)
                if cached is None:
    ...
    def snapshot() -> dict[str, Any]:
        now = time.time()
        with lock:
            data = dict(stats); data.update({"kline_calls_by_interval": dict(by_interval), "kline_hits_by_interval": dict(hits_by_interval), "kline_expired_or_missing_by_interval": dict(stale_by_interval), "kline_refreshes_by_interval": dict(refresh_by_interval), "kline_empty_returns_by_interval": dict(empty_by_interval), "entry_freshness_by_state": dict(freshness_by_state), "last_events": list(last_events)})
        manager = getattr(legacy, "market_data_manager", None)
        data["market_data_cache_source"] = "PersistentMarketDataCache" if manager is not None else "legacy_kline_cache"
        if manager is not None:
    ...
    legacy.fetch_klines = traced_fetch_klines; legacy.fetch_strategy_data = traced_fetch_strategy_data; legacy.score_symbol = traced_score_symbol
    legacy._market_data_runtime_trace_installed = True; legacy._market_data_runtime_trace_snapshot = snapshot
    return snapshot
